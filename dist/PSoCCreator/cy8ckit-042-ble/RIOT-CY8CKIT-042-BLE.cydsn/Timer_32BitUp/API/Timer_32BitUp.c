...
/**************************** PUBLIC FUNCTIONS ********************************/
void `$INSTANCE_NAME`_Init(uint32 us_per_tick)
{
    /* Set timer divider value to set tick period */
    `$INSTANCE_NAME`_CLK_TIMER_SetDividerValue(TIMER_CLK_DIVIDER_VALUE(us_per_tick));
}
